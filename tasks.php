...
<h1><?= app_icon('check-circle') ?> تسک‌ها</h1>

<!-- تسک جدید -->
<div class="card" id="addCard" style="margin-bottom:14px">
  <h2><?= app_icon('plus') ?> تسک جدید</h2>
  <form method="post">
...
<div class="card">
  <h2><?= app_icon('calendar') ?> امروز <?php if ($day['total']): ?><span class="chip c-primary" style="margin-inline-start:auto"><?= fa_num($day['done_count']) ?> از <?= fa_num($day['total']) ?></span><?php endif; ?></h2>
...
<?php if ($overdue): ?>
<div class="card mt">
  <h2><?= app_icon('pulse') ?> عقب‌افتاده‌ها</h2>
  <div class="item-list">
    <?php foreach ($overdue as $t): ?>
    <div class="item">
      <button class="check" data-task="<?= $t['id'] ?>"><?= app_icon('check') ?></button>
      <div class="grow">
        <div class="title"><?= e($t['title']) ?></div>
...
<?php if ($upcoming): ?>
<div class="card mt">
  <h2><?= app_icon('calendar') ?> روزهای آینده</h2>
  <div class="item-list">
    ...
    <div class="item">
      ...
      <form method="post" onsubmit="return confirm('حذف شود؟')"><?= csrf_field() ?>
        ...
        <button class="icon-btn" title="حذف"><?= app_icon('trash') ?></button>
      </form>
    </div>
    <?php endforeach; ?>
  </div>
