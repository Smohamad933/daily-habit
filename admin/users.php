...
if ($view) {
    ...
    ?>
    <div class="page-head">
      <h1><?= app_icon('user') ?> <?= e($view['first_name'] . ' ' . $view['last_name']) ?> <span class="muted" dir="ltr">@<?= e($view['username']) ?></span></h1>
    ...
    <div class="grid cols-2" style="align-items:start">
      ...
      <div class="card">
        <h2><?= app_icon('info') ?> دلایل انجام نشدن کارها</h2>
    ...
    <div class="card mt">
      <h2><?= app_icon('check-circle') ?> آخرین تسک‌ها</h2>
...
include dirname(__DIR__) . '/includes/header.php';
?>
<h1><?= app_icon('users') ?> مدیریت کاربران</h1>
<div class="card">
  <form class="searchbar" method="get">
    <input type="text" name="q" placeholder="جستجو در نام، نام کاربری، موبایل، شهر…" value="<?= e($q) ?>">
    <button class="btn">جستجو</button>
    <a class="btn success" href="export.php<?= $q ? '?q=' . urlencode($q) : '' ?>"><?= app_icon('download') ?> خروجی اکسل</a>
  </form>
