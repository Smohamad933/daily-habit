...
function chk(bool $ok, string $label, string $help = ''): string {
    $icon = $ok
        ? '<svg class="req-icon ok" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m5 12 4 4L19 6"/></svg>'
        : '<svg class="req-icon bad" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 6l12 12M18 6 6 18"/></svg>';
...
<?= chk($pdoMysql, 'افزونه pdo_mysql (برای پنل مانیتورینگ)', 'فقط برای نصب مانیتورینگ MySQL لازم است؛ در php.ini خط extension=pdo_mysql را فعال کنید') ?>
