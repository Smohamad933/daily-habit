...
(function () {
  ...
  function font(ctx, s) { ctx.font = s + 'px Vazirmatn, Tahoma, sans-serif'; }
  ...
  window.barChart = function (canvas, labels, values, opts) {
    ...
    values.forEach((v, i) => {
      ...
      grad.addColorStop(0, opts.color || '#22c793'); grad.addColorStop(1, '#7c5cff');
      ctx.fillStyle = grad;
      ctx.beginPath();
  ...
  window.ringChart = function (el, percent, label) {
    ...
    el.innerHTML =
      ...
      '<stop offset="0" stop-color="#7c5cff"/><stop offset="1" stop-color="#22c793"/></linearGradient></defs></svg>' +
