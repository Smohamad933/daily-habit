...
?>
<h1>🤖 ایجنت‌های هوشمند</h1>
<div class="grid dash" style="align-items:start">
  <div>
    <div class="card">
      <h2>🧠 تحلیل لحظه‌ای مربی شخصی</h2>
    ...
    <div class="card">
      ...
      <div class="item-list">
        ...
        <div class="item">
          ...
          <form method="post"><?= csrf_field() ?><input type="hidden" name="act" value="toggle"><input type="hidden" name="id" value="<?= $a['id'] ?>"><button class="btn sm ghost">⏻</button></form>
          <form method="post" onsubmit="return confirm('حذف شود؟')"><?= csrf_field() ?><input type="hidden" name="act" value="delete"><input type="hidden" name="id" value="<?= $a['id'] ?>"><button class="btn sm ghost">🗑</button></form>
        </div>
        <?php endforeach; ?>
      </div>
    </div>
