...
if ($rep['hour_missed']) {
    foreach ($rep['hour_missed'] as $h => $m) {
        ...
        $hourItems[] = ['label' => 'ساعت ' . fa_num($h), 'value' => $rate, 'suffix' => fa_num($rate) . '٪', 'color' => $rate >= 60 ? '#ef4767' : '#b57926'];
    }
...
?>
<h1><?= app_icon('chart') ?> گزارش‌ها و تحلیل هوشمند</h1>
<div class="cal-nav">
  <?php $pm = $rm == 1 ? [$ry - 1, 12] : [$ry, $rm - 1]; $nm = $rm == 12 ? [$ry + 1, 1] : [$ry, $rm + 1]; ?>
  <a class="btn sm ghost" href="reports.php?y=<?= $pm[0] ?>&m=<?= $pm[1] ?>"><?= app_icon('arrow-right') ?> ماه قبل</a>
  <b><?= j_month_name($rm) ?> <?= fa_num($ry) ?></b>
  <a class="btn sm ghost" href="reports.php?y=<?= $nm[0] ?>&m=<?= $nm[1] ?>">ماه بعد <?= app_icon('arrow-left') ?></a>
</div>
...
<div class="grid cols-2" style="align-items:start">
  <div class="card">
    <h2><?= app_icon('chart') ?> درصد هفته‌های ماه</h2>
    <div class="chart-box"><canvas id="weekBar"></canvas></div>
  </div>
  <div class="card">
    <h2><?= app_icon('pulse') ?> روند روزانه ماه</h2>
...
<div class="grid cols-2 mt" style="align-items:start">
  <div class="card">
    <h2><?= app_icon('info') ?> چرا کارها انجام نشد؟</h2>
    ...
    <div class="grid cols-2 mt">
      <div class="stat card"><div class="num" style="color:var(--bad)"><?= pct($rep['neg_percent']) ?></div><div class="lbl">سهم دلایل منفی</div></div>
  ...
  <div class="card">
    <h2><?= app_icon('clock') ?> تحلیل ساعت‌های ریزش</h2>
    <?php if ($hourItems): ?>
...
<div class="card mt">
  <h2><?= app_icon('user') ?> تیپ رفتاری این ماه</h2>
...
<div class="card mt">
  <h2><?= app_icon('repeat') ?> عملکرد عادت‌ها در این ماه</h2>
...
<div class="card mt">
  <h2><?= app_icon('activity') ?> خلاصه سریع امروز</h2>
