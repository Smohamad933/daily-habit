...
<div class="grid dash" style="align-items:start">
  <div class="card">
    <h2><?= app_icon('clock') ?> ساعت‌بندی روز</h2>
  ...
  <div>
    <div class="card">
      <h2><?= app_icon('check-circle') ?> تسک‌های این روز <?php if ($day['total']): ?><span class="chip c-primary" style="margin-inline-start:auto"><?= fa_num($day['done_count']) ?> از <?= fa_num($day['total']) ?></span><?php endif; ?></h2>
      <?php if (!$day['tasks']): ?>
        ...
        <div class="item <?= $t['status'] === 'done' ? 'done' : '' ?>">
          ...
          <div class="grow">
            ...
            <div class="meta">
              ...
              <?php if ($t['source'] === 'ai'): ?> <span class="chip c-info"><?= app_icon('bot') ?> برنامه‌ریزی مجدد</span><?php endif; ?>
            </div>
          </div>
    ...
    <div class="card mt">
      <h2><?= app_icon('clock') ?> بهترین ساعت آزاد امروز</h2>
