...
?>
<h1>🧠 تست شخصیت و سبک برنامه‌ریزی</h1>
<?php if ($result): ?>
<div class="card" style="border:2px solid var(--primary)">
  <h2>نتیجه تست شما: <?= e($result['label']) ?></h2>
  <p><?= e($result['desc']) ?></p>
  ...
  <h3 class="mt">توصیه‌های اختصاصی</h3>
  <div class="item-list">
...
<div class="card mt">
  <h2><?= $result || $last ? '🔁 شرکت دوباره در تست' : 'به ۸ سوال کوتاه پاسخ دهید' ?></h2>
  <?php if ($last && !$result): ?>
...
window.CHART_DEFS = [{ id: 'pchart', type: 'donut', items: [
  { label: 'منظم', value: <?= (int)$result['scores']['planner'] ?>, color: '#7c5cff' },
  { label: 'منعطف', value: <?= (int)$result['scores']['flexible'] ?>, color: '#f5a623' },
  { label: 'اجتماعی', value: <?= (int)$result['scores']['social'] ?>, color: '#22c793' },
  { label: 'متمرکز', value: <?= (int)$result['scores']['focused'] ?>, color: '#2f9df4' }
], opts: { center: 'شما' } }];
<?php endif; ?>
</script>
